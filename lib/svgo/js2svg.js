...
JS2SVG.prototype.createProcInst = function (instruction) {
  return `${this.config.procInstStart + instruction.name} ${instruction.body}${
...
JS2SVG.prototype.createElem = function (data) {
  ...
  let { tagOpenStart } = this.config;
  let { tagOpenEnd } = this.config;
  let { tagCloseStart } = this.config;
  let { tagCloseEnd } = this.config;
...
JS2SVG.prototype.createAttrs = function (elem) {
  ...
  elem.eachAttr(function (attr) {
    if (attr.value !== undefined) {
      attrs += ` ${attr.name}${this.config.attrStart}${String(attr.value).replace(
