...
const { encodeSVGDatauri } = require('./tools.js');
const { decodeSVGDatauri } = require('./tools.js');
...
module.exports = require('coa')
  ...
  .opt()
  .name('output')
  .title('Output file or folder (by default the same as the input), "-" for STDOUT')
  .short('o')
  .long('output')
  .arr()
  .val(function (val) {
  ...
  .opt()
  .name('precision')
  .title('Set number of digits in the fractional part, overrides plugins params')
  .short('p')
  .long('precision')
  .val(function (val) {
  ...
  .opt()
  .name('disable')
  .title('Disable plugin by name, "--disable={PLUGIN1,PLUGIN2}" for multiple plugins (*nix)')
  .long('disable')
  .arr()
  .val(function (val) {
  ...
  .opt()
  .name('enable')
  .title('Enable plugin by name, "--enable={PLUGIN3,PLUGIN4}" for multiple plugins (*nix)')
  .long('enable')
  .arr()
  .val(function (val) {
  ...
  .opt()
  .name('multipass')
  .title('Pass over SVGs multiple times to ensure all optimizations are applied')
  .long('multipass')
  .flag()
  .end()
  .opt()
  ...
  .act(function (opts, args) {
    ...
    if (input) {
      ...
      return Promise.all(input.map((file, n) => optimizeFile(config, file, output[n]))).then(
        noop,
        printErrorAndExit,
      );
...
function processDirectory(config, dir, files, output) {
  ...
  return svgFilesDescriptions.length
    ? Promise.all(
      svgFilesDescriptions.map((fileDescription) => optimizeFile(config, fileDescription.inputPath, fileDescription.outputPath)),
    )
...
function processSVGData(config, info, data, output, input) {
  ...
  return svgo.optimize(data, info).then((result) => {
    if (config.datauri) {
    ...
    return writeOutput(input, output, result.data).then(
      ...
      (error) => Promise.reject(
        new Error(
          error.code === 'ENOTDIR' ? `Error: output '${output}' is not a directory.` : error,
        ),
      ),
    );
  });
...
function printProfitInfo(inBytes, outBytes) {
  ...
  console.log(
    `${Math.round((inBytes / 1024) * 1000) / 1000} KiB${
      profitPercents < 0 ? ' + ' : ' - '
    }${chalk.green(`${Math.abs(Math.round(profitPercents * 10) / 10)}%`)} = ${Math.round(
      (outBytes / 1024) * 1000,
    ) / 1000} KiB`,
  );
}
